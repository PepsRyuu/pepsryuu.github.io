(function (global, factory) {
	...
	(global.VirtualScrollingTree = factory());
 ...
 */
var VirtualScrollingTree = function VirtualScrollingTree (options) {
    _(this).items = [];
    _(this).expansions = [{
        id: null,
...
VirtualScrollingTree.prototype.redraw = function redraw () {
    requestData.call(this);
};

VirtualScrollingTree.prototype.expand = function expand (item) {
...
VirtualScrollingTree.prototype.collapse = function collapse (item) {
    if (!isExpanded.call(this, item.parent)) {
...
VirtualScrollingTree.prototype.scrollIntoView = function scrollIntoView (item, options) {
        if ( options === void 0 ) options = { align: 'start' };

    if (!isExpanded.call(this, item.parent)) {
        throw new Error('Parent ' + item.parent + ' must be expanded to scroll ' + item.id + ' into the view');
    }

    var parent = findExpansion.call(this, item.parent);
    ...
    // 2Item 1.0 <-- offset 0
    // 3Item 1.1 <-- start 4, end 5, offset 1
    // 4   Item 1.1.0
    // 5   Item 1.1.1
    // 6Item 1.2 <-- Here we want to go
    //
    ...
    var above = parent.expansions.filter(function (e) {
...
VirtualScrollingTree.prototype.destroy = function destroy () {
...
function prepareView() {
    ...
    var contentDiv = "<div class=\"VirtualScrollingTree-content\"></div>";
    ...
    if (!_(this).smoothScrolling) {
        _(this).view.content.addEventListener('wheel', function (e) {
            _(this$1).view.scrollbar.scrollTop += e.deltaY;
        }, true);
    } 
    ...
    var lastScrollTop = 0;
    _(this).view.scrollbar.addEventListener('scroll', function (e) {
        if (_(this$1).scrollLocked) {
            return;
        }

        var newScrollTop = parseInt(_(this$1).view.scrollbar.scrollTop / _(this$1).itemHeight);

        if (lastScrollTop !== newScrollTop) {
            lastScrollTop = newScrollTop;
            if (_(this$1).smoothScrolling) {
                _(this$1).view.content.style.transform = "translateY(" + (newScrollTop * _(this$1).itemHeight) + "px)";
            }

            requestData.call(this$1);
        }
    }, true);
...
function updateViewDimensions(totalItems) {
    var visibleItems = Math.floor(_(this).parent.offsetHeight / _(this).itemHeight);
    ...
    var scrollbarWidth = totalItems < visibleItems? 0 : getNativeScrollbarWidth.call(this);
...
function requestData() {
    var visible = Math.ceil(_(this).parent.offsetHeight / _(this).itemHeight);
    var scrollIndex = Math.floor(_(this).view.scrollbar.scrollTop / _(this).itemHeight);
...
function setData(data) {
    ...
    var totalHeight = 0;
    forEachExpansion(_(this).expansions, function (expansion) {
    ...
    for (var i = 0; i < data.length; i++) {
        for (var j = 0; j < data[i].items.length; j++) {
            ...
            item.offset = _(this$1).request[i].offset + j;
    ...
    if (_(this).items.length === 0 || _(this).diffMode === 'hard') {
        ...
        while (_(this).items.length) {
            _(this$1).items.pop().el.remove();
        }


        // Once we've figured out the output, start rendering them
        output.forEach(function (data) {
            var item = createItem.call(this$1, data);
            _(this$1).view.content.appendChild(item.el);
            _(this$1).items.push(item);
        });
        ...
        var removals = [];
        var updated = [];
        _(this).items.forEach(function (item, index) {
            var found = false;
            for (var i = 0; i < output.length; i++) {
                if (output[i].id === item.id) {
                    renderItem.call(this$1, item.el, output[i], true);
                    updated.push(i);
                    found = true;
                    break;
                }
        ...
        var oldScrollTop = _(this).view.scrollbar.scrollTop;
        _(this).scrollLocked = true;

        // Remove the items.
        for (var i$1 = removals.length - 1; i$1 >= 0; i$1--) {
            var index = removals[i$1];
            _(this$1).items[index].el.remove();
            _(this$1).items.splice(index, 1);
        }

        // Insert all of the new items.
        for (var i$2 = 0; i$2 < output.length; i$2++) {
            if (updated.indexOf(i$2) === -1) {
                var item$1 = createItem.call(this$1, output[i$2]);
                 _(this$1).view.content.insertBefore(item$1.el, _(this$1).view.content.children[i$2]);
                 _(this$1).items.splice(i$2, 0, item$1);
            }
        }
...
function renderItem (element, data, updating) {
    ...
    _(this).onItemRender(element, Object.assign({}, data.original,
        {expanded: isExpanded.call(this, data.id),
        indent: calculateLevel.call(this, data),
        toggle: function () {
            // Check to see if this item is expanded or not
            var expanded = isExpanded.call(this$1, data.id);
            if (expanded) {
                collapseItem.call(this$1, data);
            } else {
                expandItem.call(this$1, data);
            }
...
function buildRequest(scrollPos, viewport) {
    // Variables
    var queries = [];
    var expanded = _(this).expansions;
